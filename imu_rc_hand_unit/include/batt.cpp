...
{
    return (double)analogBits * 4.2 / 4095;
}
uint8_t batteryPercentage(double voltage)
{
    return constrain(map(voltage, 3.7, 4.2, 0, 100), 0, 100);
}
uint8_t batteryBars(uint16_t analogBits)
{
    return map(analogBits, 0, 4095, 0, 4);
}
bool checkCharging()
{
    return digitalRead(PIN_CHARGER_STATUS) == HIGH;
}
