// Timer callbacks
void updateRSSISignal()    { updateRSSI    = true; }
void updateMenu()          { updateDisplay = true; }
void updateIMU()           { updateData    = true; }
void updateBatteryStatus() { updateBattery = true; }
void sendBLEData()         { sendData      = true; }

void setupTimer() {
  // Start the timers with the appropriate frequency (in seconds)
  menuTicker.attach(1.0 / UPDATE_RATE_MENU, updateMenu);
  rssiTicker.attach(1.0 / UPDATE_RATE_RSSI, updateRSSISignal);
